...
import { getEmails } from "../../emails/service";
...
import { ExtractedTransactionRow } from "./components/ExtractedTransactionRow";
...
function groupByHour(items: ExtractedItem[]): [string, ExtractedItem[]][] {
  ...
  items.forEach((item) => {
    ...
    const hourKey = format(ecuadorDate, "yyyy-MM-dd HH:00");
...
export default function DailyMailerExtractorPage() {
  ...
  const loadAllEmails = useCallback(async () => {
    ...
    try {
      ...
      do {
        const result = await getEmails(selectedDate, cursor);
  ...
  return (
    <div className="space-y-4">
      {/* Date + Load emails - mismo estilo que el header */}
      <div className="flex flex-col gap-4">
        ...
        {emails.length > 0 && (
          <p className="text-xs text-muted-foreground mt-0.5">
            {emails.length} emails · {highlightedEmails.length} de banco · {format(parseISO(selectedDate), "d MMM yyyy", { locale: es })}
          </p>
        )}
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
        ...
        <Card className="overflow-hidden flex flex-col gap-0">
          <CardHeader className="py-2.5 px-4 !pb-2 flex flex-row items-center justify-between gap-2 border-b">
            <CardTitle className="text-sm font-medium">Emails del día</CardTitle>
                    ...
                    >
                      ...
                      {extractingEmailId === email.id ? (
                        ...
                        <span className="text-xs text-muted-foreground shrink-0">
                          {format(new Date(email.receivedDateTime), "HH:mm")}
                        </span>
                      )}
                    </div>
                  );
